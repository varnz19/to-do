    ...
    <div className="max-w-5xl mx-auto px-6 py-8 space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-notion-card border border-notion-border rounded-xl p-4 shadow-warm-sm space-y-1">
          ...
          <div className="text-3xl font-display font-bold text-notion-text flex items-baseline gap-1">
            <span>{stats.currentStreak}</span>
            <span className="text-xs font-normal text-notion-muted font-sans italic">days active</span>
          </div>
          <p className="text-[11px] text-notion-muted italic">Daily momentum record</p>
        </div>
        ...
        <div className="bg-notion-card border border-notion-border rounded-xl p-4 shadow-warm-sm space-y-1">
          ...
          <div className="text-3xl font-display font-bold text-notion-text flex items-baseline gap-1">
            ...
            <span className="text-xs font-normal text-notion-muted font-sans italic">tasks completed</span>
          </div>
          <p className="text-[11px] text-notion-muted italic">Weekly sprint velocity</p>
        </div>
        ...
        <div className="bg-notion-card border border-notion-border rounded-xl p-4 shadow-warm-sm space-y-1">
          <div className="flex items-center justify-between">
            ...
            <Send size={16} className="text-[#235479] dark:text-[#8EB9DD]" />
          </div>
          <div className="text-3xl font-display font-bold text-notion-text flex items-baseline gap-1">
            <span>{stats.appliedThisMonth}</span>
            <span className="text-xs font-normal text-notion-muted font-sans italic">applications</span>
          </div>
          <p className="text-[11px] text-notion-muted italic">Active career pipeline</p>
        </div>

        {/* Coding & DSA Challenges Solved */}
        <div className="bg-notion-card border border-notion-border rounded-xl p-4 shadow-warm-sm space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold uppercase tracking-wider text-notion-muted">Coding Problems</span>
            <Code2 size={16} className="text-terracotta" />
          </div>
          <div className="text-3xl font-display font-bold text-notion-text flex items-baseline gap-1">
            <span>{stats.solvedChallenges}</span>
            <span className="text-xs font-normal text-notion-muted font-sans italic">of {stats.totalChallenges} solved</span>
          </div>
          <p className="text-[11px] text-notion-muted italic">DSA & system blueprints</p>
        </div>
      </div>
      ...
      {todayTotalRelevant > 0 && (
        <div className="bg-notion-card border border-notion-border rounded-xl p-4 shadow-warm-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
          <div className="space-y-0.5">
            ...
            <p className="text-notion-muted italic text-[11px]">
          ...
          <div className="flex items-center gap-3 sm:w-60">
            ...
            <span className="font-mono text-terracotta font-semibold text-xs">{progressPercent}%</span>
          </div>
        </div>
      )}
          ...
          {upcomingTasks.length > 0 && (
            <div className="bg-notion-card border border-notion-border rounded-xl p-5 shadow-warm-sm space-y-3">
              <div className="flex items-center justify-between pb-1 border-b border-notion-border/70 text-xs">
                ...
                <span className="text-notion-muted italic">Next 7 days</span>
              </div>
              ...
              <div className="space-y-2">
                {upcomingInterviews.map(job => (
                  ...
                  >
                    ...
                    <div className="text-notion-muted truncate italic text-[11px]">{job.role}</div>
                    {job.interviewDate && (
              ...
              {activityLog.length === 0 ? (
                ...
                activityLog.slice(0, 6).map(item => (
                  <div key={item.id} className="text-xs space-y-0.5 border-l-2 border-terracotta/60 pl-2.5 py-0.5">
                    ...
                    {item.details && <div className="text-[11px] text-notion-muted italic">{item.details}</div>}
